...
BAKKESMOD_PLUGIN(KickoffPlugin, "Kickoff plugin", "0.1", GameEventType::Tutorial_Free_Play)
...
void kickoffplugin_ConsoleNotifier(std::vector<std::string> params) {
	...
	{
		if (params.size() == 1) {
			BOOST_LOG_TRIVIAL(info) << "Usage: kickoff_load 0-6 (0 = cycle through all, 1-6=individual kickoff)";
			return;
		}
