...
BOOST_PYTHON_MODULE(bakkesmod)
{
	...
	class_<ConsoleWrapper>("Console", no_init).
		def("execute_command", &ConsoleWrapper::executeCommand).
		def("log", &ConsoleWrapper::log, log_overloads()).
		def("register_cvar", &ConsoleWrapper::registerCvar).
		def("get_cvar_value", &ConsoleWrapper::getCvarValue).
		def("get_cvar_int", &ConsoleWrapper::getCvarInt).
		def("get_cvar_float", &ConsoleWrapper::getCvarFloat).
		def("get_cvar_bool", &ConsoleWrapper::getCvarBool).
		def("set_cvar", &ConsoleWrapper::setCvar).
		def("log_to_chatbox", &ConsoleWrapper::logToChatbox);
	...
	class_<GameWrapper, boost::noncopyable>("GameWrapper", no_init).
		...
		def("get_f_name_by_index", &GameWrapper::GetFNameByIndex).
